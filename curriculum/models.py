...
class MathObject(models.Model):
    syntactic_structure = models.CharField(
        max_length=50, choices=SyntacticStructure.choices
    )
    type = models.CharField(
        max_length=50, choices=Behavior.choices, default=Behavior.NONE, blank=True
    )
...
class TaskClass(models.Model):
    ...
    math_object = models.ForeignKey(
        MathObject, on_delete=models.PROTECT, related_name="task_classes"
    )
    ...
    supportive_information = models.TextField(
        blank=True, help_text="Supportive Information for the task class."
    )
    procedural_information = models.TextField(
        blank=True, help_text="Procedural Information for the task class."
    )
    common_errors = models.JSONField(
        default=dict, blank=True, help_text="Anticipated misrules and misconceptions."
    )
    prerequisites = models.ManyToManyField(
        "self", through="TaskPrerequisite", symmetrical=False, blank=True
    )
    mcap_evidence_statements = models.ManyToManyField(
        "standards.MCAPEvidenceStatement", blank=True, related_name="task_classes"
    )
...
class TaskPrerequisite(models.Model):
    whole_task = models.ForeignKey(
        TaskClass, on_delete=models.CASCADE, related_name="required_prerequisites"
    )
    prerequisite_task = models.ForeignKey(
        TaskClass, on_delete=models.CASCADE, related_name="required_by"
    )
...
class LearningTask(models.Model):
    task_class = models.ForeignKey(
        TaskClass, on_delete=models.CASCADE, related_name="learning_tasks"
    )
