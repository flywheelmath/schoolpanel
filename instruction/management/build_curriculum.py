...
class Command(BaseCommand):
    ...
    def generate_macro_configs(self):
        macros_path = settings.BASE_DIR / 'instruction' / 'utils' / 'parser' / 'macros.json'

        if not macros_path.exists():
            self.stdout.write(self.style.WARNING(f"Macros file not found at {macros_path}. Skipping macro generation."))
            return
        ...
        for name, data in macros.items():
            cmd_name = name[1:]
            tex_lines.appen(f"\\newcommand{\\{cmd_name}}}[{data['args']}][1]{{{data['tex']}}}")
        ...
        self.stdount.write(self.style.SUCCESS("Successfully generated macro configurations for KaTeX and LaTeX."))
    ...
    def handle(self, *args, **kwargs):
        ...
        if not MD_DIR.exists():
            self.stdout.write(self.style.ERROR(f"Markdown directory not found: {MD_DIR}"))
            return
        ...
        for md_file in MD_DIR.glob('**/*.md'):
            ...
            if pdf_file.exists() and vue_file.exists():
                if pdf_file.stat().st_mtime >= source_mtime and vue_file.stat().st_mtime >= source_mtime:
                    skipped_count += 1
                    continue
            ...
            try:
                subprocess.run(
                    ['pdflatex', '-interaction=nonstopmode', f'-output-directory={TEX_DIR}', str(tex_file)],
                    check=True,
                ...
                self.stdout.write(self.style.ERROR(f"LaTeX compilation failed: {md_file.name}. Check the tex folder for logs."))

        self.stdout.write(self.style.SUCCESS(f"Build complete! Compiled: {compiled_count} | Skipped: {skipped_count}"))
