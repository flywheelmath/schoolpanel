...
def compile_document(raw_text, target='vue'):
    def graph_replacer(match):
        ...
        raw_points = [Lexer.parse_kwargs(m.group(1))
    ...
    def table_replacer(match):
        ...
        return VueRenderer.render_table(headers, rows, deltas, config) if target == 'vue' else TikzRenderer.render_table(headers, rows, deltas, config)

    def task_replacer(match):
        prompt = match.group('prompt').strip()
        config_str, items_str = Lexer.split_config_block(match.group('content'))

        kwargs = Lexer.parse_graph_config(config_str, {'cols': 1, 'counter': 'alph', 'shape': 'block'})
        parsed_items = Lexer.parse_task_content(items_str)

        max_rowspan = max([item['config'].get('rowspan', 1) for item in parsed_items], default=1)
        cols = kwargs['cols']
        ...
        def get_cell(r, c):
            while len(virtual_grid) <= r: virtual_grid.append([None] * cols)
        ...
        text = re.sub(r"(?P<prompt>(?:(?!\n::).)*?)\n::tasks\n(?P<content>.*?)\n::", task_replacer, raw_text, flags=re.DOTALL)
