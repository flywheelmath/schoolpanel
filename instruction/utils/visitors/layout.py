...
class DualHeightRowsGridStrategy(BaseGridRenderStrategy):
        ...
        while idx < len(cells):
            ...
            if max_line_row_span == min_line_row_span:
                if len(current_row_cells) == 1:
                    ...
                    visitor.context.set_width(
                        cell, self._get_config(cell, "col_span", 4) / 12.0
                    )
                    visitor.visit(cell)
            ...
            with visitor.indent():
                if tracks["total_left_cols"] > 0:
                    if tracks["taller_on_left"]:
                        self._emit_taller_column(
                            left_partition, tracks["total_left_cols"], visitor
                        )
                    else:
                ...
                if tracks["total_right_cols"] > 0:
                    if not tracks["taller_on_left"]:
                        self._emit_taller_column(
                            right_partition, tracks["total_right_cols"], visitor
                        )
                    else:
                        idx = self._emit_shorter_column(
                            right_partition,
    ...
    def _pack_row_line(
        self, cells: List[Node], start_idx: int
    ) -> Tuple[List[Node], int]:
    ...
    def _find_best_partition_split(self, row_cells: List[Node]) -> int:
        ...
        for i in range(len(row_cells) + 1):
            ...
            variance = abs(
                self._get_max_row_span(left_subset)
                - self._get_min_row_span(left_subset)
            ) + abs(
                self._get_max_row_span(right_subset)
                - self._get_min_row_span(right_subset)
            )

            balance = abs(
                self._sum_col_spans(left_subset) - self._sum_col_spans(right_subset)
            )
    ...
    def _get_track_assignment(
        self, left: List[Node], right: List[Node]
    ) -> Dict[str, Any]:
    ...
    def _compute_filler_rows(
        self, total_cols: int, remaining_height: int, cells: List[Node], start_idx: int
    ) -> Tuple[List[Node], int]:
        ...
        while remaining_height > 0 and temp_idx < len(cells):
            ...
            while peek_idx < len(cells):
                ...
                remaining_sum = sum(
                    self._get_config(cells[k], "col_span", 1)
                    for k in range(peek_idx, len(cells))
                )
                all_remaining_fit_in_width = (
                    inner_accumulated_cols + remaining_sum
                ) <= total_cols
    ...
    def _emit_taller_column(
        self, partition_cells: List[Node], total_cols: int, visitor: Any
    ) -> None:
        out = visitor.output
    ...
    def _emit_shorter_column(
        self,
        partition_cells: List[Node],
        total_cols: int,
        taller_max: int,
        shorter_max: int,
        cells: List[Node],
        ...
        filler_rows, updated_idx = self._compute_filler_rows(
            total_cols, remaining_height_space, cells, global_idx
        )
