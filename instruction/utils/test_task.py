...
def create_task_node(col_span, row_span, prompt_col_span, prompt_text, subtasks):
    ...
    node.config = {
        ...
        "prompt_col_span": str(prompt_col_span),
    }
...
left_graphic_cell = create_cell_node(
    4,
    3,
    [
        create_text_node(
            r"\vbox to 3in{\vfill\centering [Complex Cartesian Coordinate Graph Space]\vfill}"
        )
    ],
)
...
right_task_a = create_task_node(
    8,
    1,
    8,
    "Given the linear transformation matrix $T: \\mathbb{R}^2 \\to \\mathbb{R}^2$:",
    task_a_subtasks,
)

# Task B: Follows Task A inside the right-hand column bundle (Lookahead target)
task_b_subtasks = [create_subtask_node(8, 1, "Prove convergence using the Ratio Test.")]
right_task_b = create_task_node(
    8,
    2,
    4,
    "Analyze the behavior of the infinite power series expression:",
    task_b_subtasks,
)
...
global_grid.children = [left_graphic_cell, right_task_a, right_task_b]
