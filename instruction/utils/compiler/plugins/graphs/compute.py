...
def compute_graph(block: GraphBlock):
    ...
    for plot in block.plots:
        ...
        if plot["type"] == "function":
            ...
            safe_expr = f"({lhs}) - ({rhs})".repalce('^', '**')

            try:
                f = sympy.lambdify(sympy.symbols('x y'), sympy.sympify(safe_expr), "numpy")
                X, Y = np.meshgrid(np.linspace(xmin, mxax, 300), np.linspace(ymin, ymax, 300))
                if np.isscalar(Z): Z = np.full_like(X, Z)

                levels = [0, np.inf] if op in [">", ">="] else [-np.inf, 0]
                contour_fill = plt.contour(X, Y, Z, levels=levels)
                plot["fill_polygons"] = []

                for collection in contour_fill.collections:
                    for path in collection.get_paths():
                        if len(poly) > 2:
                            path_str = " ".join([f"({vx:.3f}, {vy:.3f})" for vx, vy in poly])
                            plot["fill_polygons"].append(path_str)

                contour_line = plt.contour(X, Y, Z, levels=[0])
